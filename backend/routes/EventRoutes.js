        ...
        if (currentStatus === 'draft') {
            ...
            if (event.eventType === 'merchandise' && req.body.merchandiseConfig) {
                if (req.body.merchandiseConfig.stock !== undefined) {
                    // Merge with existing merchandiseConfig to preserve other fields
                    allowedUpdates.merchandiseConfig = {
                        ...event.merchandiseConfig.toObject(),
                        stock: req.body.merchandiseConfig.stock
                    };
                }
            }
...
router.get('/getTrendingEvents', authMiddleware, async (req, res) => {
    try {
        ...
        for (const reg of recentRegistrations) {
            ...
            eventRegistrationCounts[eventIdStr] = (eventRegistrationCounts[eventIdStr] || 0) + 1;
        }

        const sortedEventIds = Object.entries(eventRegistrationCounts)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 5)
            .map(entry => entry[0]);
        ...
        let query = { _id: { $in: sortedEventIds } };
