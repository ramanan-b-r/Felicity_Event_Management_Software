        ...
        try {
            ...
            if (event.eventType === 'merchandise') {
                ...
                if (selectedVariants && selectedVariants.length > 0) {
                    ...
                    // Stock is NOT decremented here — it is decremented on organizer approval
                }
            }
...
router.put('/approveMerchandiseOrder', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        // Decrement stock on approval (not at purchase time)
...
router.put('/rejectMerchandiseOrder', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        // No need to restore itemsRemaining — stock is only decremented on approval, not at purchase time
        if (registration.merchandiseSelection && registration.merchandiseSelection.length > 0) {
            event.registeredCount -= 1;
            await event.save();
        }
